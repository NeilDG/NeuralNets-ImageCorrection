...
class ConcatCNN(nn.Module):
    ...
    def __init__(self):
        ...
        pool2 = nn.MaxPool2d(kernel_size=5, stride=2, padding=0)
        ...
        conv3 = nn.Conv2d(in_channels = 64, out_channels = 64, kernel_size=5, stride=2, padding=1)
        pool3 = nn.MaxPool2d(kernel_size=5, stride=2, padding=0)
    ...
    def forward(self, x):
        ...
        x2 = self.conv1_block1(x)
        x2 = self.conv2_block1(x2)
        x2 = self.conv3_block1(x2)
        x2 = self.conv4_block1(x2)
        x2 = x2.view(x2.size()[0], -1) #flatten layer
        x2 = self.fc2(x2)
        
        x3 = self.conv1_block1(x)
        x3 = self.conv2_block1(x3)
        x3 = self.conv3_block1(x3)
        x3 = self.conv4_block1(x3)
        x3 = x3.view(x3.size()[0], -1) #flatten layer
        x3 = self.fc3(x3)
        
        x4 = self.conv1_block1(x)
        x4 = self.conv2_block1(x4)
        x4 = self.conv3_block1(x4)
        x4 = self.conv4_block1(x4)
        x4 = x4.view(x4.size()[0], -1) #flatten layer
        x4 = self.fc4(x4)
        
        x5 = self.conv1_block1(x)
        x5 = self.conv2_block1(x5)
        x5 = self.conv3_block1(x5)
        x5 = self.conv4_block1(x5)
        x5 = x5.view(x5.size()[0], -1) #flatten layer
        x5 = self.fc5(x5)
        
        x6 = self.conv1_block1(x)
        x6 = self.conv2_block1(x6)
        x6 = self.conv3_block1(x6)
        x6 = self.conv4_block1(x6)
